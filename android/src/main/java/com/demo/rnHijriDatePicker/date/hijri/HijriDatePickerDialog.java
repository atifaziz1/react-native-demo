...
package com.demo.rnHijriDatePicker.date.hijri;
...
import com.demo.rnHijriDatePicker.RNDate;
...
public class HijriDatePickerDialog extends DialogFragment implements
    ...
    private HashSet<UmmalquraCalendar> highlightedDays = new HashSet<>();
    private TreeSet<UmmalquraCalendar> selectableDays = new TreeSet<>();
    private HashSet<UmmalquraCalendar> disabledDays = new HashSet<>();
    ...
    private HijriDatePickerDialog instance;
     ...
     */
    public static HijriDatePickerDialog newInstance(OnDateSetListener callBack, RNDate rnDate) {
        HijriDatePickerDialog ret = new HijriDatePickerDialog();
        ret.initialize(callBack, rnDate);
        if(rnDate.getLanguage() != null)
        ret.setLocale(new Locale(rnDate.getLanguage()));
        return ret;
    }

    public void initialize(OnDateSetListener callBack, RNDate rnDate) {
        mCallBack = callBack;

        try {
            if (rnDate.getValue() != 0) {
                rnDate.set(rnDate.getValue());
                setCurrentDate(rnDate);
            }
            if (rnDate.getMax() != 0) {
                rnDate.set(rnDate.getMax());
                setMaxDate(rnDate);
            }
            if (rnDate.getMin() != 0) {
                rnDate.set(rnDate.getMin());
                setMinDate(rnDate);
            }
    ...
    private void setMinDate(RNDate rnDate) {
        setMinDate(rnDate.getNow());
    }

    private void setMaxDate(RNDate rnDate) {
        setMaxDate(rnDate.getNow());

    }

    private void setCurrentDate(RNDate rnDate) {
        mCalendar.set(Calendar.YEAR, rnDate.year());
        mCalendar.set(Calendar.MONTH, rnDate.month());
        mCalendar.set(Calendar.DAY_OF_MONTH, rnDate.day());
//        setMinDate(rnDate.getNow());
    }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        if (savedInstanceState != null) {
            ...
            highlightedDays = (HashSet<UmmalquraCalendar>) savedInstanceState.getSerializable(KEY_HIGHLIGHTED_DAYS);
            selectableDays = (TreeSet<UmmalquraCalendar>) savedInstanceState.getSerializable(KEY_SELECTABLE_DAYS);
            disabledDays = (HashSet<UmmalquraCalendar>) savedInstanceState.getSerializable(KEY_DISABLED_DAYS);
        ...
        int viewRes = mVersion == Version.VERSION_1 ? R.layout.hdp_mdtp_hijri_date_picker_dialog : R.layout.hdp_mdtp_hijri_date_picker_dialog_v2;
    ...
    public void onClick(View v) {
        ...
        } else if (v.getId() == R.id.mdtp_hijri_date_picker_month_and_day) {
    ...
    public int getMinYear() {
        ...
        return mMinDate != null && mMinDate.get(UmmalquraCalendar.YEAR) > 0 ? mMinDate.get(UmmalquraCalendar.YEAR) : mMinYear;
    }
    ...
    public int getMaxYear() {
        ...
        return mMaxDate != null && mMaxDate.get(UmmalquraCalendar.YEAR) > 0 ? mMaxDate.get(UmmalquraCalendar.YEAR) : mMaxYear;
    }
